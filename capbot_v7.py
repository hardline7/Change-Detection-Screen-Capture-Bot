...
class ScreenMonitor(QWidget):
    ...
    def __init__(self):
        ...
        self.error_log_file = 'screen_monitor_error_log.txt'
    ...
    def create_log_files(self):
        # Ensure log files exist by creating them if they don't
    ...
    def initUI(self):
        ...
        # 여기에서 "로그 보기" 버튼을 추가합니다.
        self.view_log_btn = QPushButton('로그 보기', self)
    ...
    def log_error_to_file(self, error_message):
        with open(self.error_log_file, 'a') as log:
            log.write(f"{datetime.datetime.now()}: {error_message}\n")
    ...
    def toggle_monitoring(self):
        try:
            if not self.monitoring:
                ...
                self.log_to_file("Monitoring started.")
            else:
                self.monitoring = False
                ...
                self.log_to_file("Monitoring stopped.")
            self.save_config()
        except Exception as e:
                    ...
                    if diff_pixels > self.pixel_threshold:
                        if current_time - self.last_save_time >= self.save_cooldown:
                            ...
                            if filepath:  # Ensure filepath is valid
                                self.message_count += 1  # 메시지 카운터 증가
                                self.log_to_file(f"Change detected. Screenshot saved at {filepath}")
    ...
    def send_telegram_status(self):
        if self.monitoring and self.start_time:
            ...
            status_message = f"{hours}시간째 정상 동작 중입니다.\n\n{log_summary}"
            self.run_async_task(self.send_telegram_message(status_message))
    ...
    def generate_log_summary(self):
        try:
            ...
            return f"로그 파일을 조회하는 동안 오류가 발생했습니다: {str(e)}"
    ...
    def view_logs(self):
        try:
            if not os.path.exists(self.log_file):
                QMessageBox.information(self, "로그 없음", "로그 파일이 없습니다.")
                return
            with open(self.log_file, 'r') as log:
                log_contents = log.read()
            # 로그를 표시할 대화상자 생성
            log_dialog = QDialog(self)
            log_dialog.setWindowTitle('로그 보기')
            ...
            self.showErrorMessage("로그 보기 오류", f"로그를 표시하는 중 오류가 발생했습니다: {str(e)}")
