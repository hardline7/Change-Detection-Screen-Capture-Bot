...
class ScreenMonitor(QWidget):
    def __init__(self):
        ...
        self.save_cooldown = 2  # 스크린샷 전송 간의 최소 간격을 2초로 설정
    ...
    def initUI(self):
        self.setWindowTitle('변화감지 캡처봇 v0.0027')
        ...
        delay_layout.addWidget(QLabel('감지 후 저장 딜레이 (초):'))
        self.delay_slider = QSlider(Qt.Horizontal)
        self.delay_slider.setMinimum(1)
        self.delay_slider.setMaximum(30)  # 딜레이를 최대 30초까지 설정할 수 있도록 변경
    ...
    def update_ui_from_config(self):
        ...
        self.bot_token_input.setText(getattr(self, 'bot_token', ''))
    ...
    def update_delay(self, value):
        ...
        # 모니터링이 실행 중일 경우 타이머를 재설정
        if self.monitoring:
            self.timer.stop()
            self.timer.start(1000 * self.check_delay)
    ...
    def toggle_monitoring(self):
        try:
            if not self.monitoring:
                ...
                self.timer.start(1000 * self.delay_slider.value())  # 슬라이더 값을 직접 사용
    ...
    def save_screenshot(self, image):
        try:
            ...
            sequence_number = 1  # 시퀀스 카운트 숫자
            while True:
                filename = f'{pc_name}_{timestamp}_{sequence_number}.png'
                filepath = os.path.join(self.save_path, filename)
                if not os.path.exists(filepath):  # 파일이 존재하지 않을 때까지 시퀀스 카운트 증가
                    break
                sequence_number += 1
            image.save(filepath)
    ...
    async def send_telegram_screenshot_alert(self, filepath):
        if filepath:
            ...
            try:
                await self._send_photo(filepath)
            except Exception as e:
                self.showErrorMessage("예상치 못한 오류", f"오류가 발생했습니다: {str(e)}")

    async def _send_photo(self, filepath):
        try:
    ...
    def save_telegram_settings(self):
        ...
        try:
            ...
            self.run_async_task(bot.send_message(chat_id=chat_id, text="Telegram 설정이 성공적으로 저장되었습니다."))
        except TelegramError as e:
